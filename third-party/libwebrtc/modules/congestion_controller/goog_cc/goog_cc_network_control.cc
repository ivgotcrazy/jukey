...
NetworkControlUpdate GoogCcNetworkController::OnRoundTripTimeUpdate(
  ...
  RTC_DCHECK(!msg.round_trip_time.IsZero());
	...
	RTC_LOG(LS_INFO) << "### Update RTT:" << msg.round_trip_time;
...
NetworkControlUpdate GoogCcNetworkController::OnTransportLossReport(
	...
	// 如果打开了这个开关，则通过 transport feedback 来计算丢包率
  if (packet_feedback_only_)
    return NetworkControlUpdate();

	RTC_LOG(LS_INFO) << "### Update loss report, received:" 
		<< msg.packets_received_delta << ", lost:" << msg.packets_lost_delta;
...
void GoogCcNetworkController::UpdateCongestionWindowSize() {
  ...
  if (current_data_window_) {
	...
	//if (current_data_window_.has_value()) {
	//	RTC_LOG(LS_INFO) << "### Update congestion window size from "
	//		<< current_data_window_.value() << " to " << data_window
	//		<< ", time_window:" << time_window << ", target_rate:" 
	//		<< last_loss_based_target_rate_;
	//}
	//else {
	//	RTC_LOG(LS_INFO) << "### Update congestion window size to " << data_window;
	//}
...
NetworkControlUpdate GoogCcNetworkController::OnTransportPacketsFeedback(
  ...
  for (const auto& feedback : feedbacks) {
	...
	RTC_LOG(LS_INFO) << "### max_feedback_rtt:" << max_feedback_rtt 
  ...
  if (max_feedback_rtt.IsFinite()) {
	...
	// 基于 transport feedback 计算丢包率和 RTT
  if (packet_feedback_only_) {
  ...
  bandwidth_estimation_->SetAcknowledgedRate(acknowledged_bitrate,
	...
	if (acknowledged_bitrate.has_value()) {
		RTC_LOG(LS_INFO) << "### Acknowledged bitrate:" << acknowledged_bitrate.value();
	}
  ...
  absl::optional<DataRate> probe_bitrate =
	...
	if (probe_bitrate.has_value()) {
		RTC_LOG(LS_INFO) << "### Probe bitrate:" << probe_bitrate.value();
	}
  ...
  result = delay_based_bwe_->IncomingPacketFeedbackVector(
	...
	RTC_LOG(LS_INFO) << "### Delay based bwe result, updated:" << result.updated
		<< ", probe:" << result.probe
		<< ", target_bitrate:" << result.target_bitrate
		<< ", recovered_from_overuse:" << result.recovered_from_overuse
		<< ", delay_detector_state:" << result.delay_detector_state;
  ...
  if (recovered_from_overuse) {
    ...
    update.probe_cluster_configs.insert(update.probe_cluster_configs.end(),
                                        probes.begin(), probes.end());
		RTC_LOG(LS_INFO) << "### Recovered from overuse";
  }
...
void GoogCcNetworkController::MaybeTriggerOnNetworkChanged(
  ...
  DataRate pushback_target_rate = loss_based_target_rate;
	...
	//RTC_LOG(LS_INFO) << "MaybeTriggerOnNetworkChanged, fraction_loss:" << fraction_loss
	//	<< ", rtt:" << round_trip_time
	//	<< ", loss_based_target_rate:" << loss_based_target_rate
	//	<< ", loss_based_state:" << loss_based_state;
  ...
  if (congestion_window_pushback_controller_) {
    int64_t pushback_rate =
        congestion_window_pushback_controller_->UpdateTargetBitrate(
            (uint32_t)loss_based_target_rate.bps());
		/*RTC_LOG(LS_INFO) << "pushback_rate:" << pushback_rate;*/
    pushback_rate = std::max<int64_t>(bandwidth_estimation_->GetMinBitrate(),
                                      pushback_rate);
    pushback_target_rate = DataRate::BitsPerSec(pushback_rate);
		/*RTC_LOG(LS_INFO) << "pushback_target_rate:" << pushback_target_rate;*/
  ...
  DataRate stable_target_rate =
	...
	/*RTC_LOG(LS_INFO) << "stable_target_rate_1:" << stable_target_rate;*/

  stable_target_rate = std::min(stable_target_rate, pushback_target_rate);

	/*RTC_LOG(LS_INFO) << "stable_target_rate_2:" << stable_target_rate;*/
  ...
  if ((loss_based_target_rate != last_loss_based_target_rate_) ||
    ...
    last_loss_base_state_ = loss_based_state;
		...
		//RTC_LOG(LS_INFO) << "### last_loss_based_target_rate_:" << last_loss_based_target_rate_
		//	<< ", last_pushback_target_rate_:" << last_pushback_target_rate_
		//	<< ", last_estimated_fraction_loss_:" << (last_estimated_fraction_loss_.has_value() ? last_estimated_fraction_loss_.value() : 0)
		//	<< ", last_estimated_round_trip_time_:" << last_estimated_round_trip_time_
		//	<< ", last_stable_target_rate_:" << last_stable_target_rate_
		//	<< ", last_loss_base_state_:" << last_loss_base_state_;
    ...
    update->target_rate = target_rate_msg;
		...
		//RTC_LOG(LS_INFO) << "target_rate_msg, target_rate:" << target_rate_msg.target_rate
		//	<< ", cwnd_reduce_ratio:" << target_rate_msg.cwnd_reduce_ratio
		//	<< ", stable_target_rate:" << target_rate_msg.stable_target_rate
		//	<< ", round_trip_time:" << target_rate_msg.network_estimate.round_trip_time
		//	<< ", loss_rate_ratio:" << target_rate_msg.network_estimate.loss_rate_ratio
		//	<< ", bwe_period:" << target_rate_msg.network_estimate.bwe_period;
