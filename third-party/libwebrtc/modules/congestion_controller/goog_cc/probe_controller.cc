...
std::vector<ProbeClusterConfig> ProbeController::SetEstimatedBitrate(
    ...
    Timestamp at_time) {
	RTC_LOG(LS_VERBOSE) << "### SetEstimatedBitrate, bitrate:" << bitrate 
		<< ", bandwidth_limited_cause:" << bandwidth_limited_cause;
...
std::vector<ProbeClusterConfig> ProbeController::Process(Timestamp at_time) {
  ...
  if (TimeForAlrProbe(at_time) || TimeForNetworkStateProbe(at_time)) {
		RTC_LOG(LS_INFO) << "### Start ALR probe";
    return InitiateProbing(
...
std::vector<ProbeClusterConfig> ProbeController::InitiateProbing(
    ...
    bool probe_further) {
	RTC_LOG(LS_INFO) << "### InitiateProbing, bitrates_to_probe:" 
		<< bitrates_to_probe.size() << ", probe_further:" << probe_further;
