  ...
  class PacketSender {
   public:
    virtual ~PacketSender() = default;
		...
		virtual void SendPacket(const jukey::com::Buffer& packet,
			const PacedPacketInfo& cluster_info) = 0;

    // Should be called after each call to SendPacket().
		virtual std::vector<std::unique_ptr<RtpPacketToSend>> FetchFec() {
		...
		virtual std::vector<jukey::com::Buffer> GeneratePadding(uint32_t size) = 0;
