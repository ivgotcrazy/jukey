...
BitrateProberConfig::BitrateProberConfig(
    ...
    : min_probe_delta("min_probe_delta", TimeDelta::Millis(5)), // TODO: debug版本处理耗时较长 
...
bool BitrateProber::ReadyToSetActiveState(DataSize packet_size) const {
  ...
  switch (probing_state_) {
    ...
    case ProbingState::kInactive:
      ...
      // probe asap.
			RTC_LOG(LS_NONE) << "### packet_size:" << packet_size 
				<< ", RecommendedMinProbeSize():" << RecommendedMinProbeSize() 
				<< ", config_.min_packet_size:" << config_.min_packet_size.Get();
      return packet_size >=
