...
HRESULT CreateVideoDeviceSource(IMFMediaSource * *ppSource)
{
	...
	*ppSource = NULL;
	...
	hr = ppDevices[1]->ActivateObject(IID_PPV_ARGS(&pSource));
	if (FAILED(hr))
		abort();
...
int main(int argc, char** argv)
{
	...
	hr = SetDeviceFormat(ppSource, 4);   //I need to configure the camera to format 6.
	if (FAILED(hr))
		abort();
