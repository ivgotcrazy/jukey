...
// 
//------------------------------------------------------------------------------
EncodedVideoSendProcessor::EncodedVideoSendProcessor(PipelineProcessorMgr* mgr)
	: PipelineProcessorBase(mgr->m_factory, PPMT_PROCESSOR, 
			PPST_VIDEO_ENCODED_SENDER, mgr)
	, m_pl_proc_mgr(mgr)
{
...
// 
//------------------------------------------------------------------------------
EncodedVideoSendProcessor::~EncodedVideoSendProcessor()
{
...
// 
//------------------------------------------------------------------------------
com::ErrCode EncodedVideoSendProcessor::Init(const com::MediaStream& stream,
	const std::string& addr)
{
	...
	if (!m_decode_element) {
		LOG_ERR("Create video decode element failed!");
	...
	auto encode_element = m_pipeline->AddElement(CID_VIDEO_ENCODE, nullptr);
	if (!encode_element) {
	...
	auto send_element = m_pipeline->AddElement(CID_VIDEO_SEND, prop.get());
	if (!send_element) {
	...
	LOG_INF("Init EncodedVideoSendProcessor success");
...
// 
//------------------------------------------------------------------------------
bool EncodedVideoSendProcessor::AllocSinkPin(const std::string& stream_id,
	com::ElementPin& pin)
{
	if (!m_decode_element) {
...
// 
//------------------------------------------------------------------------------
void EncodedVideoSendProcessor::ReleaseSinkPin(const com::ElementPin& pin)
{
	LOG_INF("{}", __FUNCTION__);
}
...
// 
//------------------------------------------------------------------------------
bool EncodedVideoSendProcessor::ContainStream(const std::string& stream_id)
{
