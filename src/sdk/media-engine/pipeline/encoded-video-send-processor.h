...
// 
//==============================================================================
class EncodedVideoSendProcessor : public PipelineProcessorBase
{
public:
	EncodedVideoSendProcessor(PipelineProcessorMgr* mgr);
	~EncodedVideoSendProcessor();

	com::ErrCode Init(const com::MediaStream& stream, const std::string& addr);

	// IPipelineProcessor
	virtual bool AllocSinkPin(const std::string& stream_id, com::ElementPin& pin) override;
...
typedef std::shared_ptr<EncodedVideoSendProcessor> EncodedVideoSendProcessorSP;
